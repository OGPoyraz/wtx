...
import { URL } from "url";
import { info, error } from "../lib/log.js";

const __dirname = new URL(".", import.meta.url).pathname;

const COMMON_MARKDOWN = `# wtx — Worktree Manager
...
${COMMON_MARKDOWN}`,
  cursor: COMMON_MARKDOWN,
  claude: COMMON_MARKDOWN,
};

const SKILL_DESCRIPTIONS: Record<string, string> = {
  opencode: "AI skill with frontmatter for opencode",
  cursor: "AI skill for Cursor IDE",
  claude: "AI skill for Claude Code",
};

export function registerSkillCommand(program: Command) {
  const skillCmd = program.command("skill").description("Manage AI skills for wtx");

  skillCmd
    .command("list")
    .description("List available AI platforms")
    .action(() => {
      for (const [platform, desc] of Object.entries(SKILL_DESCRIPTIONS)) {
        info(`  ${platform.padEnd(10)} - ${desc}`);
      }
    });

  skillCmd
    .command("show <platform>")
    .description("Show the skill file content for a platform")
    .action((platform: string) => {
      if (!(platform in EMBEDDED_SKILLS)) {
        error(`Unknown platform: ${platform}`);
        process.exit(1);
      }
      process.stdout.write(EMBEDDED_SKILLS[platform] + "\\n");
    });

  skillCmd
    .command("path <platform>")
    .description("Print the path to the skill file")
    .action((platform: string) => {
      if (!(platform in EMBEDDED_SKILLS)) {
        error(`Unknown platform: ${platform}`);
        process.exit(1);
      ...
      let skillPath = path.join(projectRoot, "skills", `${platform}.md`);
      
      if (fs.existsSync(skillPath)) {
        process.stdout.write(skillPath + "\n");
      } else {
        error(`Skill file not found on disk at ${skillPath}.`);
        error(`Since you are running a compiled binary, use 'wtx skill show ${platform}' to see the embedded content instead.`);
        process.exit(1);
      }
    });
}
